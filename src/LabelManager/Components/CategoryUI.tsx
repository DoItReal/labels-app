...
export function Category({ filterCategory, setFilterCategory }: { filterCategory: string[], setFilterCategory: (arg:string[])=>void}) {
    ...
    return (
        <FormControl size="small" sx={{ m: 1,minWidth:'10em', maxWidth: '20em', display:'flex', flexWrap:'wrap' }}>
            ...
            <Select
                ...
                renderValue={(selected) => (
                    <Box sx={{
                        display: 'flex', flexWrap: 'wrap', gap: 0.1 }}>
                        {selected.map((value) => (
                            <Chip key={value} label={value} size='small' sx={{bgcolor:'orange'}} />
                        ))}
                    </Box>
                )}
                MenuProps={MenuProps}
